...
function TodoItem({Index,Status, Todo}, visibility) {
  ...
  return (
    <motion.button 
    className={`${status? "bg-green-300 text-red-800 ":"bg-pink-300"} flex justify-evenly w-[60%] rounded-full h-fit p-4 mx-32 mb-4`}
    initial={{opacity:0, x:"-100%"}}
    animate={{opacity:1, x:0}}
    transition={{duration:2, delay:0.5, stiffness:160, type:"spring"}}
    >
        <input  type="checkbox" className='w-6 h-6 my-auto mr-8'  value={Status} onChange={updateStatus}  />
